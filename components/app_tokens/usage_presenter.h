...
/* The burn-rate rows. An "exhausts" forecast gets a local clock time only
 * when tokens->has_tz_offset_min; otherwise the detail is relative to the
 * week's reset ("RUNS OUT IN 1D 3H"), because the device has no time zone
 * of its own and a UTC clock dressed as local is a lie. */
